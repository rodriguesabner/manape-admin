...
const Suggestions = (props: SuggestionsProps) => {
    ...
    useEffect(() => {
        async function getItems() {
            ...
            onValue(items, (snapshot) => {
                ...
                const items: any = menus.map(menu => {
                ...
                const uniqueItems = items.flat().filter((item: any, index: number, self: any) =>
                    index === self.findIndex((t: any) => (
                        t.name === item.name
                    ))
                );

                const orderByName = uniqueItems.sort((a: any, b: any) => {
                    return a.name.localeCompare(b.name)
                });

                setSuggestions(orderByName)
            });
        }
    ...
    const filteredItems = suggestions.filter((item: any) => {
